...
import { 
  Building2, Calendar, Clock, Target, TrendingUp, 
  BookOpen, Code, Layers, Users, CheckCircle, AlertCircle
} from 'lucide-react';
...
export default function PlacementDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      // Load profile
      const profileRes = await fetch(`http://localhost:8000/api/placement/profile/${profileId}`);
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-green-50 to-teal-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-3xl font-bold text-gray-900">
                {Object.values(questions.topics).reduce((sum, t) => sum + t.question_count, 0)}
              </span>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          <div className="space-y-6">
            {Object.entries(questions.topics).map(([topic, data]) => (
              <div key={topic} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 mb-2">{topic}</h3>
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getFrequencyColor(data.frequency)}`}>
        ...
        {/* Interview Rounds */}
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Interview Rounds</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {profile.round_structure.map((round, idx) => (
              <div key={idx} className="border-2 border-gray-200 rounded-lg p-4 hover:border-green-500 transition">
                ...
                <p className="text-xs text-gray-500 mt-1">{round.duration} minutes</p>
              </div>
            ))}
          </div>
        </div>
